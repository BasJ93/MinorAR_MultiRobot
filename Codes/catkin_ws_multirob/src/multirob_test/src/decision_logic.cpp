...
int calcGoalDistance(float x, float y, float qx, float qy, float qz, float qw)
{
  ros::NodeHandle n;
  ros::ServiceClient client = n.serviceClient<nav_msgs::GetPlan>("move_base_node/make_plan");
  nav_msgs::GetPlan plan;

  plan.request.start.header.frame_id = "base_footprint";
  plan.request.start.header.stamp = ros::Time::now();
  plan.request.start.pose.position.x = 0.0;
